  ...
  <el-card class="box-card">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="handleEditRoleName(scope.$index, scope.row)">修改</el-button>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" @close="onDialogClose()" width="80%">
      <el-form ref="roleForm" :rules="rules" :model="roleForm" label-width="120px">
        ...
        <el-form-item v-if="dialogTitle !== '角色授权'" label="备注" prop="remark">
  ...
  <el-tab-pane :key="key" :label="role.permissionName">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="info" @click="doAdd()" v-if="dialogTitle=='修改角色名称'">保存</el-button>
...
<script>
import { getRoleList, getAllPermissiion, rolesave } from '@/api/permission'
export default {
    data() {
        return {
            form: {
    ...
    methods: {
        async doAdd() {
            if (this.roleForm.roleName) {
                this.form.id = this.roleForm.id
                this.form.roleName = this.roleForm.roleName
                this.form.status = this.roleForm.status
                this.form.remark = this.roleForm.remark
            }
            if (
                !this.form.roleName ||
                !this.form.status ||
            ...
            this.form.status = this.roleForm.status === '停用' ? '0' : (this.roleForm.status === '启用' ? '1' : this.form.status)
        ...
        onSearch() {
            getRoleList(this.page, this.pageSize, this.searchData.roleName)
                .then(res => {
        ...
        handleEditRoleName(index, row) {
            ...
            this.roleForm.remark = String(this.roleForm.remark)
            this.dialogVisible = true
